...
curl -i -X POST \
  ...
  --data 'upstream_url=http://localhost:8080/API-Engine-web/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://192.168.99.100:8001/apis/devapiengine.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
...
curl -i -X POST \
  ...
  --data 'upstream_url=http://localhost:8080/API-Engine-auth/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://192.168.99.100:8001/apis/devapiengineauth.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
    --data "config.methods=GET,HEAD,PUT,PATCH,POST,DELETE" \
    --data "config.headers=Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, apikey, Authorization" \
...
curl -i -X POST \
  ...
  --data 'upstream_url=http://localhost:8080/API-Engine-auth/v1/gtw/tokens/' \
  --data 'strip_request_path=true'
