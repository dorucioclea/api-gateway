...
#register api engine
curl -i -X POST \
  --url http://devasu1062.dev.digant.antwerpen.local:8001/apis/ \
  --data 'request_path=/apiengine/v1' \
  --data 'name=apiengine.v1' \
  --data 'upstream_url=http://devasu1060.dev.digant.antwerpen.local/API-Engine-web/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/apis/apiengine.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
...
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/consumers \
...
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/consumers/marketplace.v1/key-auth \
    --data "key=229e2ea08ba94919c9d221cdf3be1f72"
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/consumers/int.marketplace.v1/key-auth \
    --data "key=229e2ea08ba94919c9d221cdf3be1f71"
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/consumers/ext.marketplace.v1/key-auth \
    --data "key=229e2ea08ba94919c9d221cdf3be1f73"
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/consumers/acpaas.consent_app_nodejs.v1/key-auth \
...
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/consumers/pub.publisher.v1/key-auth \
    --data "key=***REMOVED***"
...
curl -i -X POST \
  ...
  --data 'upstream_url=http://devasu1060.dev.digant.antwerpen.local/API-Engine-auth/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/apis/apiengineauth.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
    --data "config.methods=GET,HEAD,PUT,PATCH,POST,DELETE" \
    --data "config.headers=Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, apikey, Authorization" \
...
curl -i -X POST \
  ...
  --data 'upstream_url=http://devasu1060.dev.digant.antwerpen.local/API-Engine-auth/v1/gtw/tokens/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://devasu1062.dev.digant.antwerpen.local:8001/apis/gatewaykeys/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
    --data "config.methods=GET,HEAD,PUT,PATCH,POST,DELETE" \
    --data "config.headers=Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, apikey, Authorization" \
