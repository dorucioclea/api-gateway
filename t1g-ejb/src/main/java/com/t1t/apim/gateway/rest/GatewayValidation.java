...
public class GatewayValidation {
    ...
    public Policy validate(Policy policy, PolicyType type, String... optionalPrefixId) throws PolicyViolationException, StorageException {
        ...
        switch (policies) {
            ...
            case BASICAUTHENTICATION:
                return validateBasicAuth(policy);
            case CORS:
                return validateCORS(policy);
            case FILELOG:
                return validateFileLog(policy);
            case HTTPLOG:
                return validateHTTPLog(policy);
            case UDPLOG:
                return validateUDPLog(policy);
            case TCPLOG:
                return validateTCPLog(policy);
            case IPRESTRICTION:
                return validateIPRestriction(policy);
            case KEYAUTHENTICATION:
                return validateKeyAuth(policy);
            case OAUTH2:
                Policy pol = validateOAuth(policy, (optionalPrefixId.length > 0 ? optionalPrefixId[0] : null));
                return pol;
            case RATELIMITING:
                return validateRateLimiting(policy);
            case REQUESTSIZELIMITING:
                return validateRequestSizeLimiting(policy);
            case REQUESTTRANSFORMER:
                return validateRequestTransformer(policy);
            case RESPONSETRANSFORMER:
                return validateResponseTransformer(policy);
            case SSL:
                return validateSSL(policy);
            case ANALYTICS:
                return validateAnalytics(policy);
            case JWT:
                return validateJWT(policy);
            case JWTUP:
                return validateJWTUp(policy);
            case ACL:
            ...
            case LDAPAUTHENTICATION:
                return validateLDAP(policy);
            case JSONTHREATPROTECTION:
                return validateJsonThreatProtection(policy);
            case HAL:
                return policy;
            case AWSLAMBDA:
                return validateAWSLambda(policy);
            case DATADOG:
                return validateDataDogPolicy(policy);
            default:
                throw new PolicyViolationException("Unknown policy " + policy);
        }
    }
        ...
        for (KongPluginOAuthScope scope : scopes) {
            if (!StringUtils.isEmpty(scope.getScope())) {
                ...
                if (!StringUtils.isEmpty(optionalPrefixId) && !scope.getScope().startsWith(optionalPrefixId)) {
                    ...
                    if (scope.getScope().contains("."))
                        strippedScope = scope.getScope().substring(scope.getScope().lastIndexOf(".") + 1);
    ...
    public synchronized Policy validateRequestTransformer(Policy policy) {
        ...
        req.getRemove().getBody().stream().forEach((val) -> {
            if (val != null) res.getRemove().getBody().add(val);
        });
        req.getRemove().getHeaders().stream().forEach((val) -> {
            if (val != null) res.getRemove().getHeaders().add(val);
        });
        req.getRemove().getQuerystring().stream().forEach((val) -> {
            if (val != null) res.getRemove().getQuerystring().add(val);
        });
        //remove empty lists
    ...
    public synchronized Policy validateResponseTransformer(Policy policy) {
        ...
        req.getAdd().getJson().stream().forEach((val) -> {
            if (val != null) res.getAdd().getJson().add(val);
        });
        req.getRemove().getHeaders().stream().forEach((val) -> {
            if (val != null) res.getRemove().getHeaders().add(val);
        });
        req.getRemove().getJson().stream().forEach((val) -> {
            if (val != null) res.getRemove().getJson().add(val);
        });
        //remove empty lists
        if (res.getAdd().getHeaders().size() == 0) res.getAdd().setHeaders(null);
    ...
    public synchronized Policy validateCORS(Policy policy) {
        ...
        req.getExposedHeaders().stream().forEach(val -> {
            if (!StringUtils.isEmpty(val)) res.getExposedHeaders().add(val);
        });
        req.getHeaders().stream().forEach(val -> {
            if (!StringUtils.isEmpty(val)) res.getHeaders().add(val);
        });
        req.getMethods().stream().forEach(val -> {
            if (val != null) res.getMethods().add(val);
        });
    ...
    public synchronized Policy validateHTTPLog(Policy policy) {
        ...
        if (StringUtils.isEmpty(req.getHttpEndpoint()))
    ...
    public synchronized Policy validateIPRestriction(Policy policy) {
        ...
        if (isEmptyList(req.getBlacklist()) && isEmptyList(req.getWhitelist()))
            throw new PolicyDefinitionInvalidException("At least one value should be provided.");
        if (isNotEmptyList(req.getBlacklist()) && isNotEmptyList(req.getWhitelist()))
            throw new PolicyDefinitionInvalidException("You cannot provide both blacklist and whitelist values.");
        ...
        req.getBlacklist().stream().forEach(val -> {
            if (!StringUtils.isEmpty(val)) res.getBlacklist().add(val);
        });
        req.getWhitelist().stream().forEach(val -> {
            if (val != null) res.getWhitelist().add(val);
        });
        Policy responsePolicy = new Policy();
        responsePolicy.setPolicyImpl(policy.getPolicyImpl());
        responsePolicy.setPolicyJsonConfig(gson.toJson(res));
    ...
    public synchronized Policy validateKeyAuth(Policy policy) {
        ...
        if (req.getKeyNames() != null && req.getKeyNames().size() > 0) {
            ...
            throw new PolicyViolationException("Default applies already or you should provide at least one key name.");
    }
    ...
    public synchronized Policy validateAnalytics(Policy policy) {
        ...
        if (StringUtils.isEmpty(req.getServiceToken()))
            throw new PolicyViolationException("Form was not correctly filled in.");
