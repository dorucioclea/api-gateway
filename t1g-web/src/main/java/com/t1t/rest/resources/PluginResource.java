...
import com.t1t.apim.facades.PluginFacade;
import com.t1t.apim.rest.resources.IPluginResource;
...
public class PluginResource implements IPluginResource {
    ...
    @ApiResponses({
            @ApiResponse(code = 200, response = PluginBean.class, message = "An api-gateway plugin.")
    })
    @GET
    @Path("/{pluginId}")
    @Produces(MediaType.APPLICATION_JSON)
