...
public class APIMScimIntegrationTest {
            ...
            throws Exception
    {
        ...
        final String host = "https://idp.t1t.be:9443/wso2/scim";
        final String userName = "admin";
        final String password = "admin";

        final SCIMService client = new SCIMService(URI.create(host), userName, password);

        final APIMScimIntegrationTest clientTest = new APIMScimIntegrationTest(client);
        Resources<UserResource> resultset = client.getUserEndpoint().query("userNameEqmichallis");
        Iterator<UserResource> iterator = resultset.iterator();
        while (iterator.hasNext()){
            UserResource testuser = iterator.next();
            System.out.println("USERNAME = "+testuser.getUserName());
            testuser.setTitle("Sometesttitle");
...
/*        clientTest.testGetUser();
    ...
    public void testEditUser() throws Exception
    {
        ...
        final UserResource userFromResponse = client.getUserEndpoint().update(user);
    ...
    {
        final UserResource user = createUser();
        final UserResource userFromResponse = client.getUserEndpoint().get(user.getId());
