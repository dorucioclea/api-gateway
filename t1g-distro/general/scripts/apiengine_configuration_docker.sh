...
curl -i -X POST \
  ...
  --data 'request_path=/dev/apigateway/v1' \
  --data 'name=devapigateway.v1' \
  --data 'upstream_url=http://localhost:8080/t1g-web/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://192.168.99.100:8001/apis/devapigateway.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
...
curl -X POST http://192.168.99.100:8001/consumers/dev.marketplace.v1/key-auth \
    --data "key=6b8406cc81fe4ca3cc9cd4a0abfb97c2"
curl -X POST http://192.168.99.100:8001/consumers/int.marketplace.v1/key-auth \
    --data "key=***REMOVED***"
curl -X POST http://192.168.99.100:8001/consumers/ext.marketplace.v1/key-auth \
    --data "key=***REMOVED***"

#enable keyauth for publisher and return API key, result should be captured and is the API key for the given consumer
curl -X POST http://192.168.99.100:8001/consumers/dev.publisher.v1/key-auth \
    --data "key=***REMOVED***"
...
curl -i -X POST \
  --url http://192.168.99.100:8001/apis/ \
  --data 'request_path=/dev/apigatewayauth/v1' \
  --data 'name=devapigatewayauth.v1' \
  --data 'upstream_url=http://localhost:8080/gateway-auth/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://192.168.99.100:8001/apis/devapigatewayauth.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
    --data "config.methods=GET,HEAD,PUT,PATCH,POST,DELETE" \
    --data "config.headers=Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, apikey, Authorization" \
...
curl -i -X POST \
  ...
  --data 'upstream_url=http://localhost:8080/t1g-auth/v1/gtw/tokens/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://192.168.99.100:8001/apis/gatewaykeys/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
    --data "config.methods=GET,HEAD,PUT,PATCH,POST,DELETE" \
    --data "config.headers=Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, apikey, Authorization" \
