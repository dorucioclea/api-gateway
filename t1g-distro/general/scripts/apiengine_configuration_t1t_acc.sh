...
curl -i -X POST \
  ...
  --data 'request_path=/acc/apigateway/v1' \
  --data 'name=acc.apigateway.v1' \
  --data 'upstream_url=http://accapi.t1t.be/t1g-web/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://accapim.t1t.be:8001/apis/acc.apigateway.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
...
curl -i -X POST \
  ...
  --data 'name=acc.apigatewayauth.v1' \
  --data 'upstream_url=http://accapi.t1t.be/t1g-auth/v1/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://accapim.t1t.be:8001/apis/acc.apigatewayauth.v1/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
    --data "config.methods=GET,HEAD,PUT,PATCH,POST,DELETE" \
    --data "config.headers=Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, apikey, Authorization" \
...
curl -i -X POST \
  ...
  --data 'upstream_url=http://accapi.t1t.be/t1g-auth/v1/gtw/tokens/' \
  --data 'strip_request_path=true'

#enable CORS
curl -X POST http://accapim.t1t.be:8001/apis/gatewaykeys/plugins \
    --data "name=cors" \
    --data "config.origin=*" \
    --data "config.methods=GET,HEAD,PUT,PATCH,POST,DELETE" \
    --data "config.headers=Accept, Accept-Version, Content-Length, Content-MD5, Content-Type, Date, apikey, Authorization" \
